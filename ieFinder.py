...
parser=argparse.ArgumentParser(description='Finds and logs .ie domain names by checking certfiicate transparency logs in near realtime.')
    ...
    if message['message_type']=="certificate_update":
        ...
        if len(all_domains)==0:
            ...
            if domain.endswith(".ie"):                           
                clean_domain_name = domain.split('.')[len(domain.split('.'))-2] + '.' + domain.split('.')[len(domain.split('.'))-1]
                www_domain_name = 'www.' + domain.split('.')[len(domain.split('.'))-2] + '.' + domain.split('.')[len(domain.split('.'))-1]
...
def main():
    ...
    print(f'{Fore.GREEN}The .ie do{Fore.WHITE}main na{Fore.LIGHTYELLOW_EX}me finder{Fore.RESET}{Style.DIM} | {hyperlink("https://github.com/senf666/iefinder","Github")}\n')
